...
const ViewStory = ({viewStory , setViewStory}) => {
    ...
    const renderContent = () =>{
        switch (viewStory.media_type) {
            ...
            case 'video':
                return (
                    <video onEnded={()=> setViewStory(null)} src={viewStory.media_url} controls autoPlay className='max-h-screen' alt="" />
                );
            case 'text':
                return (
    ...
    style={{backgroundColor: viewStory.media_type === 'text' ? viewStory.background_color: '#000000'}}>
      ...
      {/* User info - Top List */}
        <div className='absolute top-4 left-4 flex items-center space-x-3 p-2 px-4 sm:p-4 sm:px-8 backdrop-blur-2xl rounded bg-black/50'>
            <img src={viewStory.user?.profile_picture} className='ize-7 sm:size-8 rounded-full object-cover border border-white' alt="" />
